#!/usr/bin/env python
"""Batch generation driver for the Qwen3-TTS arms (E10): benchmark texts x voice
...
Example:
  ...
  python scripts/run_generation_qwen.py \
      --benchmark data/benchmark/pilot.jsonl --references data/references/references.jsonl \
      --experiment QE1 --out outputs/v31_qwen/QE1_pilot --seed 0 \
      --voice-filter 'ref_(female|male)_01$'

The script re-executes itself under the fork's pinned venv
(third_party/Qwen3-TTS/lora_finetuning/.venv/bin/python) when started from any
other interpreter, so callers do not need to know about that environment.
...
import qwen3tts as adapter  # noqa: E402  (torch-free at import)
import run_generation as RG  # noqa: E402  (torch-free: schema loaders + SchemaError)


def reexec_under_fork_venv():
    """Restart this script with the fork's venv python (idempotent).
    ...
    if os.path.abspath(sys.prefix) == venv_root or os.environ.get('QWEN_ADAPTER_NO_REEXEC'):
        return
    if not os.path.exists(venv_py):
        sys.exit('fork venv python not found: {} (uv sync --locked in {}/lora_finetuning)'.format(
            venv_py, adapter.FORK_ROOT))
    env = dict(os.environ)
    env['PYTHONPATH'] = adapter.FORK_ROOT + (os.pathsep + env['PYTHONPATH'] if env.get('PYTHONPATH') else '')
...
def main():
    ...
    ap.add_argument('--mode', choices=['native'], default='native',
                    help='only native single-trajectory generation exists for Qwen3-TTS (E10 prereg)')
    ap.add_argument('--experiment', required=True, help='QE1|QE2P|QE3P|... (recorded in manifests)')
    ap.add_argument('--out', required=True)
    ap.add_argument('--seed', type=int, nargs='+', default=None, help='overrides config seeds list')
    ap.add_argument('--config', default=os.path.join(ROOT, 'configs', 'models', 'qwen3tts_base.yaml'))
    ap.add_argument('--weights', default=None,
                    help='override model.model_dir (an SFT checkpoint directory for Q-E2P/Q-E3P)')
    ...
    if not os.environ.get('CUDA_VISIBLE_DEVICES'):
        print('WARNING: CUDA_VISIBLE_DEVICES is not set; E10 runs on an explicitly '
              'granted card (2 now, 0/1 when free)', file=sys.stderr)
    ...
    reexec_under_fork_venv()   # everything below needs torch + qwen_tts
    ...
    t_load = time.time()
    tts = adapter.load_qwen3tts(model_cfg)
    log('model loaded in {:.1f}s; repo_revision={}; config check: {}; mem: {}'.format(
        time.time() - t_load, adapter.repo_revision(model_cfg.get('fork_root', adapter.FORK_ROOT)),
        adapter.check_generate_defaults(tts), adapter.free_mem_info()))
