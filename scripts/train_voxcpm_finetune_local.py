#!/usr/bin/env python3
# A22-voxcpm (E12): LOCAL COPY of third_party/VoxCPM/scripts/train_voxcpm_finetune.py
# (upstream commit f5a1c6a6b901bc732e20f0d59a369f6829ad717a stays READ-ONLY; every deviation
# from it is recorded in reports/voxcpm_local_edits.patch). Local edits:
#   (1) sys.path points at the upstream src + this project's src;
#   (2) offset-aware data loading (src/voxcpm_train/data_offsets.py): the manifest's
#       ``audio`` column stays a path string with offset_start/offset_end slice columns,
#       decoded via soundfile seek-read + librosa resample to 16 kHz mono (the official
#       datasets.Audio cast can only decode whole files; our windows are slices of the
#       project's source flacs, prereg E12 forbids re-cut audio copies on the crisis SSD);
#   (3) disk-safe checkpointing: env VOXCPM_SAVE_OPTIMIZER=0 skips optimizer.pth/
#       scheduler.pth on INTERMEDIATE saves (~16 GB of AdamW state per save for the 2B
#       model; crash-recovery then restarts from model weights with a fresh optimizer --
#       the same documented compromise as the E10 Qwen rotating checkpoints), the
#       ``latest`` pointer is a SYMLINK instead of a full copytree duplicate, and env
#       VOXCPM_SAVE_TOTAL_LIMIT=N rotates old step_* folders after a successful save.

import sys
from pathlib import Path

UPSTREAM_ROOT = Path("third_party/VoxCPM")
project_root = Path(__file__).parent.parent          # .
sys.path.insert(0, str(project_root / "src"))        # voxcpm_train.*
sys.path.insert(0, str(UPSTREAM_ROOT / "src"))       # upstream voxcpm package (read-only)
...
# LOCAL EDIT (2): offset-aware replacements for the two data entry points; everything
# else in voxcpm.training is used unchanged.
from voxcpm_train.data_offsets import (
    LatentBatchProcessor,
    build_dataloader_latents,
    build_dataloader_offsets,
    load_audio_text_datasets_offsets,
)


# ---------------------------------------------------------------------------------- #
# LOCAL EDIT (6): gradient checkpointing + chunked local-module computation.
# Upstream has NO activation checkpointing; a 48 GB card OOMs in backward already on a
# 120-s unit (packed len 1898, P2 probe 2026-09-02): fp32 master weights (9.2 GB) +
# grads (9.2 GB) + AdamW state (18.3 GB) leave ~10 GB, while stored activations for the
# 28-layer base LM, 8-layer residual LM, 12-layer local encoder and 12-layer LocDiT
# reach ~26 GB at seq 1898 and grow linearly. Enabled with VOXCPM_GRAD_CKPT=1:
#   (a) every MiniCPMDecoderLayer of base_lm/residual_lm runs under
#       torch.utils.checkpoint (use_reentrant=False) in training;
#   (b) VoxCPMLocEnc processes patches in chunks of VOXCPM_ENC_CHUNK (default 512)
#       patches, each chunk checkpointed -- exact: the encoder is per-patch independent
#       (local_encoder.py:66-78, is_causal=False, CLS per patch);
#   (c) UnifiedCFM.compute_loss is replaced by a chunked replica (VOXCPM_DIT_CHUNK,
#       default 512 patches/chunk, estimator call checkpointed). Loss-identical to the
#       upstream masked mean: adaptive_loss_weighting uses p=0.0 (unified_cfm.py:141-147)
#       so weights == tgt_mask, and sum(mask*loss)/clamp(mask.sum()) decomposes over
#       chunks exactly; RNG draws (cfg/noise/r/t/z) happen per chunk, i.e. a different
#       but equally distributed sample than upstream's single big draw.
# The wrappers pass through untouched in eval / no-grad mode.
# ---------------------------------------------------------------------------------- #
def _apply_memory_optimizations(m):
    import torch.nn.functional as F
    import torch.utils.checkpoint as _ckpt

    enc_chunk = int(os.environ.get("VOXCPM_ENC_CHUNK", "512"))
    dit_chunk = int(os.environ.get("VOXCPM_DIT_CHUNK", "512"))

    def _wrap_layer(layer):
        orig = layer.forward

        def fw(hidden_states, position_emb, is_causal, _orig=orig, _layer=layer):
            if _layer.training and torch.is_grad_enabled() and hidden_states.requires_grad:
                return _ckpt.checkpoint(_orig, hidden_states, position_emb, is_causal,
                                        use_reentrant=False)
            return _orig(hidden_states, position_emb, is_causal)

        layer.forward = fw

    for lyr in m.base_lm.layers:
        _wrap_layer(lyr)
    for lyr in m.residual_lm.layers:
        _wrap_layer(lyr)

    enc = m.feat_encoder
    enc_orig = enc.forward

    def enc_fw(x, _orig=enc_orig, _enc=enc):
        if not (_enc.training and torch.is_grad_enabled()):
            return _orig(x)
        outs = []
        for i in range(0, x.shape[1], enc_chunk):
            outs.append(_ckpt.checkpoint(_orig, x[:, i:i + enc_chunk], use_reentrant=False))
    ...
    def chunked_compute_loss(x1, mu, cond=None, tgt_mask=None, progress=0.0, _cfm=cfm):
        if not (_cfm.training and torch.is_grad_enabled()):
            return type(_cfm).compute_loss(_cfm, x1, mu, cond=cond, tgt_mask=tgt_mask,
                                           progress=progress)
        assert not _cfm.mean_mode, "chunked compute_loss replica covers mean_mode=False only"
        N = x1.size(0)
        if cond is None:
            cond = torch.zeros_like(x1)
        denom = torch.clamp(tgt_mask.sum(), min=1.0) if tgt_mask is not None else None
        noise_prob = _cfm.noise_cond_prob_range[0] + progress * (
            _cfm.noise_cond_prob_range[1] - _cfm.noise_cond_prob_range[0])
        total = x1.new_zeros(())
        n_loss_elems = 0
        for i in range(0, N, dit_chunk):
            sl = slice(i, min(i + dit_chunk, N))
            x1c, muc, condc = x1[sl], mu[sl], cond[sl]
            b = x1c.size(0)
            if _cfm.training_cfg_rate > 0:
                cfg_mask = torch.rand(b, device=x1c.device) > _cfm.training_cfg_rate
                muc = muc * cfg_mask.view(-1, 1)
            noisy_mask = torch.rand(b, device=x1c.device) > (1.0 - noise_prob)
            condc = condc + noisy_mask.view(-1, 1, 1) * torch.randn_like(condc) * _cfm.noise_cond_scale
            r, t = _cfm.sample_r_t(x1c, ratio_r_neq_t=0.0)
            r = r.reshape(b)
            t = t.reshape(b)
            t_ = t.detach().clone()
            z = torch.randn_like(x1c)
            y = (1 - t_.view(-1, 1, 1)) * x1c + t_.view(-1, 1, 1) * z
            v = z - x1c
            u_pred = _ckpt.checkpoint(_cfm.estimator, y, muc, t, condc, t - r,
                                      use_reentrant=False)
            losses = F.mse_loss(u_pred, v.detach(), reduction="none").mean(dim=1)  # [b, P]
            if tgt_mask is not None:
                w = tgt_mask[sl].squeeze(1)
                total = total + (w.detach() * losses).sum()
            else:
                total = total + losses.sum()
                n_loss_elems += losses.numel()
        if denom is not None:
            return total / denom
        return total / max(n_loss_elems, 1)

    cfm.compute_loss = chunked_compute_loss
    print("[local] gradient checkpointing armed: base_lm 28 + residual_lm 8 layers, "
          f"enc_chunk={enc_chunk}, dit_chunk={dit_chunk}", file=sys.stderr)
...
def train(
    ...
    # LOCAL EDIT (11): series seed convention (E12 P3, Lead greenlight): upstream
    # hardcodes Accelerator(seed=42); VOXCPM_TRAIN_SEED overrides (P3 runs use 0).
    accelerator = Accelerator(amp=True, seed=int(os.environ.get("VOXCPM_TRAIN_SEED", "42")))
    ...
    # LOCAL EDIT (2): offset-aware manifests (audio stays a path string + offsets)
    train_ds, val_ds, _sr = load_audio_text_datasets_offsets(
    ...
    # LOCAL EDIT (2): offset-aware dataloaders (same collate_fn/packer as upstream).
    # LOCAL EDIT (9): when the manifest carries a 'latents' column, audio comes as
    # precomputed AudioVAE latents (long arm; the encoder's first conv over a 568-s unit
    # is a single 4.33 GiB fp32 allocation that OOMs next to the training statics --
    # see data_offsets.py). The short arm keeps the upstream on-GPU encode path.
    use_latents = "latents" in train_ds.column_names
    if accelerator.rank == 0:
        print(f"[local] latent path: {use_latents}", file=sys.stderr)
    if use_latents:
        train_loader = build_dataloader_latents(
            train_ds, accelerator=accelerator, batch_size=batch_size,
            num_workers=num_workers, drop_last=True)
        val_loader = (
            build_dataloader_latents(
                val_ds, accelerator=accelerator, batch_size=batch_size,
                num_workers=num_workers, drop_last=False)
            if val_ds is not None else None)
    else:
        train_loader = build_dataloader_offsets(
            train_ds,
    ...
    # LOCAL EDIT (9): latent manifests skip the on-GPU VAE encode entirely
    _bp_cls = LatentBatchProcessor if use_latents else BatchProcessor
    batch_processor = _bp_cls(
    ...
    if use_latents:
        # the VAE is still wanted on-device for the validation demo decode
        base_model.audio_vae.to(accelerator.device)
    ...
    # LOCAL EDIT (6): activation checkpointing + chunked local modules (see header)
    if os.environ.get("VOXCPM_GRAD_CKPT", "0") == "1":
        _apply_memory_optimizations(unwrapped_model)
    ...
    with tracker.live():
        for step in range(start_step, num_iters):
            ...
            if step % log_interval == 0 or step == num_iters - 1:
                ...
                # LOCAL EDIT (5): peak VRAM in every logged step (P3 health monitoring)
                if torch.cuda.is_available():
                    loss_values["peak_vram_gb"] = round(torch.cuda.max_memory_allocated() / 2**30, 2)
                    loss_values["peak_vram_reserved_gb"] = round(torch.cuda.max_memory_reserved() / 2**30, 2)
                # LOCAL EDIT (8): per-block grad norms (smoke gate: every trainable block
                # must receive nonzero gradients). Grads are zeroed only at the top of the
                # NEXT loop iteration, so they are still available here.
                if os.environ.get("VOXCPM_LOG_BLOCK_GRADS", "0") == "1":
                    blocks = {"base_lm": 0.0, "residual_lm": 0.0, "feat_encoder": 0.0,
                              "feat_decoder": 0.0, "other": 0.0}
                    for pname, p in unwrapped_model.named_parameters():
                        if p.grad is None:
                            continue
                        g = float(p.grad.detach().norm(2)) ** 2
                        for b in ("base_lm", "residual_lm", "feat_encoder", "feat_decoder"):
                    ...
                    for b, v in blocks.items():
                        loss_values[f"gnorm/{b}"] = round(v ** 0.5, 6)
            ...
            # LOCAL EDIT (4): upstream saves a full checkpoint at step 0 (an untrained
            # copy of the base model, ~25 GB with optimizer state); skip saves until
            # at least one optimizer step has been taken past the resume point.
            if step > start_step and (step % save_interval == 0 or step == num_iters - 1) and accelerator.rank == 0:
...
def load_checkpoint(model, optimizer, scheduler, save_dir: Path, rank: int = 0):
    ...
    if state_path.exists():
        ...
        # LOCAL EDIT (10): model-only rotating checkpoints (VOXCPM_SAVE_OPTIMIZER=0)
        # carry no scheduler.pth; fast-forward the fresh cosine schedule to the resume
        # step so the LR does not restart from warmup (the optimizer itself restarts
        # fresh -- the documented E10-style compromise).
        if not scheduler_loaded and resume_step > 0:
            for _ in range(resume_step):
                scheduler.step()
            if rank == 0:
                print(f"Scheduler fast-forwarded to step {resume_step} (no scheduler.pth)", file=sys.stderr)
        if rank == 0:
...
def save_checkpoint(
    ...
    # LOCAL EDIT (7): VOXCPM_DISABLE_SAVE=1 skips checkpointing entirely (VRAM/throughput
    # probes only; a 2B model save is a 9.2 GB disk write on the crisis SSD).
    if os.environ.get("VOXCPM_DISABLE_SAVE", "0") == "1":
        print(f"[local] VOXCPM_DISABLE_SAVE=1: skipping checkpoint at step {step}", file=sys.stderr)
        return
    ...
    if lora_cfg is not None:
        ...
        if SAFETENSORS_AVAILABLE:
            # LOCAL EDIT (13, owner RAM rule 2026-09-05): stream the state dict to disk in
            # 64-MB chunks instead of save_file's full ~9.2 GB host-RAM copy
            # (src/voxcpm_train/safetensors_stream.py; per-tensor bytes identical, loads
            # with safetensors unchanged). VOXCPM_STREAM_SAVE=0 restores save_file.
            if os.environ.get("VOXCPM_STREAM_SAVE", "1") == "1":
                from voxcpm_train.safetensors_stream import save_file_streaming
                n_written = save_file_streaming(state_dict, str(folder / "model.safetensors"))
                print(f"[local] model.safetensors streamed: {n_written/2**30:.2f} GiB", file=sys.stderr)
            else:
    ...
    # LOCAL EDIT (3a): optimizer/scheduler state (~16 GB fp32 AdamW for the 2B model) is
    # written only when VOXCPM_SAVE_OPTIMIZER != 0 (default 1 = upstream behaviour).
    # With 0, crash-recovery restarts from model weights with a fresh optimizer (the
    # documented E10-style rotating-checkpoint compromise).
    if os.environ.get("VOXCPM_SAVE_OPTIMIZER", "1") != "0":
    ...
    # LOCAL EDIT (12): make the checkpoint DURABLE before `latest` / rotation touch
    # anything. Root cause of the 2026-09-02 host-restart data loss: model.safetensors
    # sat in the page cache when the host hard-restarted, the files came back truncated
    # ("incomplete metadata, file not fully covered"), and rotation had already deleted
    # the previous good checkpoint. fsync every file + the directory FIRST; only then
    # repoint latest and rotate. VOXCPM_FSYNC_SAVE=0 opts out (probes only).
    if os.environ.get("VOXCPM_FSYNC_SAVE", "1") == "1":
        import time as _time
        _t0f = _time.time()
        for _fp in folder.iterdir():
            if _fp.is_file():
                _fd = os.open(str(_fp), os.O_RDONLY)
                try:
                    os.fsync(_fd)
                finally:
                    os.close(_fd)
        _dfd = os.open(str(folder), os.O_RDONLY)
        try:
            os.fsync(_dfd)
        finally:
            os.close(_dfd)
        print(f"[local] checkpoint {folder.name} fsynced in {_time.time()-_t0f:.1f}s", file=sys.stderr)

    # LOCAL EDIT (3b): `latest` is a SYMLINK to the newest step folder instead of a full
    # copytree duplicate (upstream doubles every checkpoint's disk footprint).
    ...
    try:
        if latest_link.is_symlink() or latest_link.exists():
            if latest_link.is_dir() and not latest_link.is_symlink():
                ...
                latest_link.unlink()
        latest_link.symlink_to(folder.name)
    except Exception:
        print(f"Warning: failed to update latest checkpoint at {latest_link}", file=sys.stderr)

    # LOCAL EDIT (3c): rotate old step_* folders, keeping the newest
    # VOXCPM_SAVE_TOTAL_LIMIT (default: keep everything = upstream behaviour).
    limit = int(os.environ.get("VOXCPM_SAVE_TOTAL_LIMIT", "0"))
    if limit > 0:
        step_folders = sorted(
            (d for d in save_dir.iterdir() if d.is_dir() and not d.is_symlink() and d.name.startswith("step_")),
            key=lambda d: int(d.name.split("_")[1]),
        )
        for old in step_folders[:-limit]:
            try:
                shutil.rmtree(old)
                print(f"Rotated old checkpoint {old}", file=sys.stderr)
            except Exception as e:
                print(f"Warning: failed to rotate {old}: {e}", file=sys.stderr)
