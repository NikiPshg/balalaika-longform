...
DEFAULT_MODEL_PATH = "Qwen/Qwen3-TTS-12Hz-1.7B-CustomVoice"
DEFAULT_TEXT_SOURCES = [
    "/workspace/Qwen3-TTS-streaming-custom/lora_finetuning/tts_bench_phrases_ru.txt"
]
LOGGER = logging.getLogger("batch_infer_single_speaker")
...
def main():
    parser = argparse.ArgumentParser(description="Batch inference for a no-LoRA Qwen3-TTS CustomVoice/single-speaker model.")
    parser.add_argument("--model_path", default=DEFAULT_MODEL_PATH, help="Hugging Face repo id or local model directory.")
    parser.add_argument("--speaker", required=True, help="Speaker id/name supported by the CustomVoice model.")
    parser.add_argument("--instruct", default=None, help="Optional instruction passed to generate_custom_voice.")
    parser.add_argument("--text_source", action="append", default=None, help="Text source path. Can be passed multiple times.")
    parser.add_argument("--output_dir", default="/workspace/Qwen3-TTS-streaming-custom/generated_single_speaker")
    ...
    LOGGER.info("Starting batch single-speaker inference")
    LOGGER.info("Model: %s", args.model_path)
    LOGGER.info("Speaker: %s", args.speaker)
    ...
    LOGGER.info(
        "Loading model with device=%s dtype=%s attn_implementation=%s",
        args.device,
        args.dtype,
    ...
    if model_type != "custom_voice":
        raise SystemExit(
            f"Expected a CustomVoice/single-speaker model for generate_custom_voice, got tts_model_type={model_type!r}."
        )

    supported_speakers = tts.get_supported_speakers()
    if supported_speakers is not None:
        LOGGER.info("Supported speakers: %s", ", ".join(supported_speakers))
        if args.speaker.lower() not in {speaker.lower() for speaker in supported_speakers}:
            raise SystemExit(f"Speaker {args.speaker!r} is not supported. Supported: {supported_speakers}")
