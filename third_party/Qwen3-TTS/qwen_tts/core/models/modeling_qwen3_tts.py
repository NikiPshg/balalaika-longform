...
class Qwen3TTSForConditionalGeneration(Qwen3TTSPreTrainedModel, GenerationMixin):
    ...
    def stream_generate_pcm(
        ...
        for step_idx in range(max_frames):
            ...
            current_decode_window_frames = decode_window_frames
            current_use_optimized_decode = use_optimized_decode
            if not first_chunk_emitted:
                if first_chunk_emit_every > 0:
                    current_emit_every = first_chunk_emit_every
                if first_chunk_decode_window_frames > 0:
                    current_decode_window_frames = first_chunk_decode_window_frames
                if first_chunk_use_optimized_decode is not None:
                    current_use_optimized_decode = first_chunk_use_optimized_decode
            ...
            start = max(0, len(codes_buffer) - current_decode_window_frames)
            ...
            window, _ = _add_ref_code_context(
                window_codes, ref_code_context, ref_code_frames, current_decode_window_frames
            )

            # Use optimized decode path when available
            # Pass pad_to_size to ensure fixed tensor size for torch.compile
            if current_use_optimized_decode and hasattr(self.speech_tokenizer, 'decode_streaming'):
                wavs, sr = self.speech_tokenizer.decode_streaming(
                    window.to(self.talker.device),
                    use_optimized=True,
                    pad_to_size=current_decode_window_frames,
                )
            else:
            ...
            # Blend chunk boundaries to smooth sliding-window re-decode artifacts.
        ...
        if remaining_frames > 0:
            ...
            # Blend the final boundary and taper the stream ending.
            blend_samples = max(overlap_samples, MIN_BLEND_SAMPLES)
            ...
            if blend_samples > 0 and len(wav) > 0:
