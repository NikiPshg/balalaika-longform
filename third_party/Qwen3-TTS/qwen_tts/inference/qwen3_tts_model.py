...
class Qwen3TTSModel:
    ...
    def from_pretrained(
        ...
        processor_source_path: Optional[str] = None,
        **kwargs,
    ) -> "Qwen3TTSModel":
        """
        ...
        Args:
            ...
            processor_source_path (Optional[str]):
                Optional fallback tokenizer source used only when the
                checkpoint does not include a root `preprocessor_config.json`.
            **kwargs:
        ...
        try:
            ...
            if "preprocessor_config.json" not in str(exc):
                raise

            tokenizer_source = processor_source_path or pretrained_model_name_or_path
            tokenizer_kwargs = {
                "fix_mistral_regex": True,
                "trust_remote_code": kwargs.get("trust_remote_code", False),
            }
            for passthrough_key in ("cache_dir", "revision", "token", "local_files_only"):
                if passthrough_key in kwargs:
                    tokenizer_kwargs[passthrough_key] = kwargs[passthrough_key]
    ...
    def stream_generate_voice_clone(
        ...
        first_chunk_decode_window_frames: int = 0,
        first_chunk_use_optimized_decode: Optional[bool] = None,
        # Optimization
        ...
        **kwargs,
        ...
        Args:
            ...
            first_chunk_emit_every: Optional emit interval used only before the first yield.
            first_chunk_decode_window_frames: Optional decode window used only before the first yield.
            first_chunk_use_optimized_decode: Optional decode mode used only before the first yield.
        ...
        for chunk, sr in self.model.stream_generate_pcm(
            ...
            first_chunk_decode_window_frames=first_chunk_decode_window_frames,
            first_chunk_use_optimized_decode=first_chunk_use_optimized_decode,
    ...
    def stream_generate_custom_voice(
        ...
        **kwargs,
        ...
        Args:
            ...
            language: Language for synthesis.
            instruct: Optional instruction text for supported models.
            non_streaming_mode: Whether to use non-streaming text input mode.
            emit_every_frames: Emit PCM chunk every N codec frames.
            decode_window_frames: Window size for decoding (longer = better quality, more latency).
            overlap_samples: Overlap samples for crossfade between chunks.
        ...
        if isinstance(speaker, list):
            raise ValueError("stream_generate_custom_voice only supports single speaker, not batch")
        if isinstance(language, list):
            raise ValueError("stream_generate_custom_voice only supports single language, not batch")
        if isinstance(instruct, list):
            raise ValueError("stream_generate_custom_voice only supports single instruct, not batch")
